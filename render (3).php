...
/* ── Scroll-in animation (view.js progressive enhancement) ──────────── */
@media (prefers-reduced-motion: no-preference) {
	.wing-services-list__card.wing-anim {
		opacity: 0;
		transform: translateY(20px);
	}
	.wing-services-list__card.wing-anim.is-visible {
		animation: wingFadeUp .5s cubic-bezier(.4,0,.2,1) var(--wing-delay, 0ms) forwards;
	}
	@keyframes wingFadeUp {
		to { opacity: 1; transform: translateY(0); }
	}
}

/* ── Accessibility: list reset (frontend uses <ul>) ─────────────────── */
.wing-services-list__grid {
	list-style: none;
	padding: 0;
	margin: 0;
...
.wing-services-list__card-link:focus-visible {
